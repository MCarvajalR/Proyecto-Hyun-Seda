package es.clubpenguin.sample.hexagonalarchitecture.infrastructure.adapters.output.customizedexception;
...
import es.clubpenguin.sample.hexagonalarchitecture.infrastructure.adapters.output.customizedexception.data.response.ExceptionResponse;
...
public class CustomizedExceptionAdapter extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<Object> handleUserNotFoundException(CategoryNotFound ex, WebRequest request) {
        ExceptionResponse exceptionResponse = new ExceptionResponse(LocalDateTime.now(), ex.getMessage(), Arrays.asList(request.getDescription(false)));
